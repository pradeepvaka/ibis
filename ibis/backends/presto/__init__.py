...
class Backend(
    ...
    @contextlib.contextmanager
    def _safe_raw_sql(self, query: str | sge.Expression) -> Iterator[Any]:
    ...
    def do_connect(
        ...
        **kwargs,
        ...
        host
            Hostname of the Presto server
        port
            Port of the Presto server
        database
            Catalog to use on the Presto server
        schema
            Schema to use on the Presto server
        source
            Application name passed to Presto
        timezone
            Timezone to use for the connection
        kwargs
            Additional keyword arguments passed directly to the
            `prestodb.dbapi.connect` API.
        ...
        >>> con = ibis.connect(f"presto:///{catalog}/{schema}")

        Connect using a URL

        >>> con = ibis.connect(f"presto://user@localhost:8080/{catalog}/{schema}")
    ...
    def from_connection(cls, con, /) -> Backend:
        """Create an Ibis client from an existing connection to a Presto database.

        Parameters
        ----------
        con
            An existing connection to a Presto database.
        """
        new_backend = cls()
    ...
    def _get_schema_using_query(self, query: str) -> sch.Schema:
        ...
        return sch.Schema(
            {
                name: type_mapper.from_string(presto_type).copy(
                    # presto types appear to be always nullable
                    nullable=True
                )
                for name, _, _, _, presto_type, *_ in info
            }
        )
    ...
    def create_table(
        ...
        *,
        ...
        database
            ...
            "database")`.
        temp
            This parameter is not yet supported in the Presto backend, because
            Presto doesn't implement temporary tables
        overwrite
        ...
        if temp:
            raise NotImplementedError(
                "Temporary tables are not supported in the Presto backend"
            )
        ...
        if obj is not None:
            ...
            # cast here because presto doesn't allow specifying a schema in
    ...
    def _fetch_from_cursor(self, cursor, schema: sch.Schema) -> pd.DataFrame:
        ...
        from ibis.backends.presto.converter import PrestoPandasData
        ...
        try:
            ...
            cursor.close()
            raise
        df = PrestoPandasData.convert_table(df, schema)
        return df
    ...
    def _register_in_memory_table(self, op: ops.InMemoryTable) -> None:
        ...
        if null_columns := schema.null_fields:
            raise com.IbisTypeError(
                "Presto cannot yet reliably handle `null` typed columns; "
