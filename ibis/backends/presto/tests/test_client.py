...
from ibis.backends.presto.tests.conftest import (
    PRESTO_HOST,
    PRESTO_PASS,
    PRESTO_PORT,
    PRESTO_USER,
...
@pytest.fixture
def tmp_name(con):
    name = util.gen_name("test_presto")
    yield name
    con.drop_table(name, force=True)


def test_table_properties(tmp_name):
    con = ibis.presto.connect(database="hive", schema="default")
...
def test_create_table_timestamp():
    con = ibis.presto.connect(database="memory", schema="default")
    schema = ibis.schema(
        dict(zip(string.ascii_letters, map("timestamp({:d})".format, range(10))))
    )
    table = util.gen_name("presto_temp_table")
...
def test_connect_uri():
    con = ibis.connect(
        f"presto://{PRESTO_USER}:{PRESTO_PASS}@{PRESTO_HOST}:{PRESTO_PORT}/memory/default"
    )
