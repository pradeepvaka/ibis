...
PRESTO_USER = os.environ.get(
    "IBIS_TEST_PRESTO_USER", os.environ.get("PRESTO_USER", "user")
)
PRESTO_PASS = os.environ.get(
    "IBIS_TEST_PRESTO_PASSWORD", os.environ.get("PRESTO_PASSWORD", "")
)
PRESTO_HOST = os.environ.get(
    "IBIS_TEST_PRESTO_HOST", os.environ.get("PRESTO_HOST", "localhost")
)
PRESTO_PORT = int(
    os.environ.get("IBIS_TEST_PRESTO_PORT", os.environ.get("PRESTO_PORT", "8080"))
)


class TestConf(ServiceBackendTest):
    # presto rounds half to even for double precision and half away from zero
    ...
    def preload(self):
        ...
        for path in self.test_files:
            ...
            dirname = path.with_suffix("").name
            # copy from minio container to presto minio host
            subprocess.run(
                [
                    "docker",
                    "compose",
    ...
    def _load_tpc(self, *, suite, **_) -> None:
        """Create views of data in the TPC-H catalog that ships with Presto.
        ...
        with self.connection.begin() as con:
            ...
            for stmt in sqls:
                raw_sql = stmt.sql("presto", pretty=True)
                con.execute(raw_sql)
    ...
    return (
        sge.Create(
            ...
            this=sge.Schema(
                ...
                expressions=schema.to_sqlglot_column_defs("presto"),
            ),
            properties=sge.Properties(
                expressions=[
                    sge.Property(
                        this="external_location",
                        value=sge.convert(f"s3a://presto/{suite_name}/{name}"),
                    ),
