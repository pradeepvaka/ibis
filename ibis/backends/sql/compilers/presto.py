...
from ibis.backends.sql.datatypes import PrestoType
from ibis.backends.sql.dialects import Presto
...
class PrestoCompiler(SQLGlotCompiler):
    __slots__ = ()

    dialect = Presto
    type_mapper = PrestoType
    ...
    def visit_Correlation(self, op, *, left, right, how, where):
        if how == "sample":
            raise com.UnsupportedOperationError(
                "Presto does not support `sample` correlation"
            )
    ...
    def visit_ArrayFilter(self, op, *, arg, param, body, index):
        ...
        placeholder = sg.to_identifier("__presto_filter__")
        ...
        zipped = self.f.zip_with(
            ...
            # seems like presto won't ever actually address the limit
            self.f.sequence(0, self.f.cardinality(arg) - 1),
            sge.Lambda(
                this=self.cast(
                    sge.Struct(
    ...
    def visit_InSubquery(self, op, *, rel, needle):
        ...
        # presto is very strict about structs
    ...
    def visit_ExtractMicrosecond(self, op, *, arg):
        # presto only seems to store milliseconds, but the result of formatting
    ...
    def visit_TemporalDelta(self, op, *, part, left, right):
        # presto truncates _after_ the delta, whereas many other backends
    ...
    def _make_interval(self, arg, unit):
        ...
        if isinstance(arg, sge.Literal):
            # force strings in interval literals because presto requires it
    ...
    def visit_RegexExtract(self, op, *, arg, pattern, index):
        # sqlglot doesn't support the third `group` argument for presto so work
